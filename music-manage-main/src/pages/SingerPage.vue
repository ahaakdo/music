...
export default {
    ...
    methods:{
        ...
        handleAvatorSuccess(response) {
            const singerId = this.registerForm.id;
            if (response.code === 1) {
                this.notify('上传成功', 'success');
                const newImageUrl = response.pic;
                const singerIndex = this.tableData.findIndex(singer => singer.id === singerId);
                if (singerIndex !== -1) {
                    this.$set(this.tableData, singerIndex, { ...this.tableData[singerIndex], pic: newImageUrl });
                }
                // 关闭上传窗口
        ...
        addSinger(){
            ...
            .then(res => {
                if(res.code == 1){
                    ...
                    // 获取新插入歌手的 ID
                    const newSingerId = res.singer.id;

                    // 在成功添加歌手后，设置 registerForm.id
                    this.registerForm.id = newSingerId;
