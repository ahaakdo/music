<template>
    <div class="table">
        ...
        <el-dialog title="上传用户图片" :visible.sync="uploadDialogVisible" width="400px" center>
            <div style="display: flex; flex-direction: column; align-items: center;">
                <el-upload :action="uploadUrl(registerForm.id)" :before-upload="beforeAvatorUpload" :on-success="handleAvatorSuccess">
                    <el-button size="mini">上传用户图片</el-button>
                </el-upload>
                <el-button size="mini" @click="uploadDialogVisible = false" style="margin-top: 10px;">暂不上传</el-button>
            </div>
        </el-dialog>
...
export default {
    ...
    data(){
        return{
            ...
            uploadDialogVisible: false, // 上传图片窗口是否显示
            registerForm:{      //添加框
                username: '',
                password: '',
                sex: '1',
    ...
    methods:{
        ...
        handleAvatorSuccess(response) {
            const consumerId = this.registerForm.id;
            if (response.code === 1) {
                this.notify('上传成功', 'success');
                const newImageUrl = response.avator;
                const consumerIndex = this.tableData.findIndex(consumer => consumer.id === consumerId);
                if (consumerIndex !== -1) {
                    this.$set(this.tableData, consumerIndex, { ...this.tableData[consumerIndex], avator: newImageUrl });
                }
                // 关闭上传窗口
                ...
                if(valid){
                    ...
                    .then(res => {
                        if(res.code == 1){
                            ...
                            // 获取新插入用户的 ID
                            const newConsumerId = res.consumer.id;

                            // 在成功添加用户后，设置 registerForm.id
                            this.registerForm.id = newConsumerId;
