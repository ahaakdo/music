<template>
    <div class="table">
        ...
        <el-dialog title="上传歌单图片" :visible.sync="uploadDialogVisible" width="400px" center>
...
export default {
    ...
    methods:{
        ...
        handleAvatorSuccess(response) {
            const songListId = this.registerForm.id;
            if (response.code === 1) {
                this.notify('上传成功', 'success');
                const newImageUrl = response.pic;
                const songListIndex = this.tableData.findIndex(songList => songList.id === songListId);
                if (songListIndex !== -1) {
                    this.$set(this.tableData, songListIndex, { ...this.tableData[songListIndex], pic: newImageUrl });
                }
                // 关闭上传窗口
        ...
        addSongList(){
            ...
            .then(res => {
                if(res.code == 1){
                    ...
                    // 获取新插入歌单的 ID
                    const newSongListId = res.songList.id;

                    // 在成功添加歌单后，设置 registerForm.id
                    this.registerForm.id = newSongListId;
