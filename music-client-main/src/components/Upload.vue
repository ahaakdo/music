    ...
    <div class="upload">
        ...
        <div class="section">
            <el-upload drag :action="uploadUrl()" :show-file-list="false" :on-success="handleAvatorSuccess"
                ...
                <div slot="tip" class="wenzi">只能上传jpg/png文件，且不能超过10MB</div>
            </el-upload>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        beforeAvatorUpload(file){
            const isJPG = file.type=='image/jpeg'|| file.type === 'image/png'; // 支持JPG和PNG格式;
            const isLt10M = file.size /1024/1024<10;
            if(!isJPG){
                this.notify('上传头像图片只能是JPG/PNG格式','error');
                return false;
            }
            if(!isLt10M){
